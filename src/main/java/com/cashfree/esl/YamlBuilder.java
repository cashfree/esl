...
/**
 * A builder for creating YAML payloads. Modified the SDK class to customize
 * JsonGenerator via JsonBuilder.
 *
 * @author Pragalathan M <pragalathanm@gmail.com>
 */
public class YamlBuilder extends GroovyObjectSupport implements Writable {

    private final groovy.json.JsonBuilder jsonBuilder;

    public YamlBuilder(groovy.json.JsonBuilder builder) {
    ...
    /**
     * Named arguments can be passed to the YAML builder instance to create a
     * root YAML object
     * <p>
     * Example:
    ...
    /**
     * A list of elements as arguments to the YAML builder creates a root YAML
     * array
     * <p>
     * Example:
     * <pre><code class="groovyTestCase">
    ...
    /**
     * Varargs elements as arguments to the YAML builder create a root YAML
     * array
     * <p>
     * Example:
     * <pre><code class="groovyTestCase">
    ...
    /**
     * A collection and closure passed to a YAML builder will create a root YAML
     * array applying the closure to each object in the collection
     * <p>
     * Example:
     * <pre><code class="groovyTestCase">
     * class Author {
     ...
     * </code></pre>
     *
     * @param c a closure whose method call statements represent key / values of
     *          a YAML object
     * @return a map of key / value pairs
     */
    ...
    /**
     * A method call on the YAML builder instance will create a root object with
     * only one key whose name is the name of the method being called. This
     ...
     * </code></pre>
     * <p>
     * If you use named arguments and a closure as last argument, the key/value
     * pairs of the map (as named arguments) and the key/value pairs represented
     * in the closure will be merged together &mdash; the closure properties
     * overriding the map key/values in case the same key is used.
     * <pre><code class="groovyTestCase">
     * def yaml = new groovy.yaml.YamlBuilder()
     ...
     * </code></pre>
     * <p>
     * The empty args call will create a key whose value will be an empty YAML
     * object:
     * <pre><code class="groovyTestCase">
     * def yaml = new groovy.yaml.YamlBuilder()
     * yaml.person()
     ...
     * conformant YAML payload string
     * <p>
     * Example:
     * <pre><code class="groovyTestCase">
     * def yaml = new groovy.yaml.YamlBuilder()
    ...
    /**
     * The YAML builder implements the <code>Writable</code> interface, so that
     * you can have the builder serialize itself the YAML payload to a writer.
     * <p>
     * Example:
     * <pre><code class="groovyTestCase">
     * def yaml = new groovy.yaml.YamlBuilder()
